...
public class KeyRebuilderTest
{
    ...
    public void Key1Test()
    {
        ...
        Assert.That(builder.RebuildKey(empty, 0, id) == empty, Is.True);
        Assert.That(builder.RebuildKey(idb, 0, id) == idb, Is.True);

        Assert.That(builder.RebuildKey(null, 0, id).SequenceEqual(idb), Is.True);
        Assert.That(builder.RebuildKey(Array.Empty<byte>(), 0, id).SequenceEqual(idb), Is.True);
        Assert.That(builder.RebuildKey(new byte[idb.Length - 1], 0, id).SequenceEqual(idb), Is.True);
        Assert.That(builder.RebuildKey(new byte[idb.Length + 1], 0, id).SequenceEqual(idb), Is.True);

        var key = builder.RebuildKey(null, 1, id);
        Assert.That(key.SequenceEqual(idb), Is.True);
        Assert.That(builder.RebuildKey(key, 0, id) == key, Is.True);
        Assert.That(builder.RebuildKey(key, 1, id) == key, Is.True);
    ...
    public void Key2Test()
    {
        ...
        Assert.That(builder.RebuildKey(empty, 0, prefix, id) == empty, Is.True);
        Assert.That(builder.RebuildKey(pid, 0, prefix, id) == pid, Is.True);

        Assert.That(builder.RebuildKey(null, 0, prefix, id).SequenceEqual(pid), Is.True);
        Assert.That(builder.RebuildKey(Array.Empty<byte>(), 0, prefix, id).SequenceEqual(pid), Is.True);
        Assert.That(builder.RebuildKey(new byte[pid.Length - 1], 0, prefix, id).SequenceEqual(pid), Is.True);
        Assert.That(builder.RebuildKey(new byte[pid.Length + 1], 0, prefix, id).SequenceEqual(pid), Is.True);

        var key = builder.RebuildKey(null, 3, prefix, id);
        Assert.That(key.SequenceEqual(pid), Is.True);
        Assert.That(builder.RebuildKey(key, 0, prefix, id) == key, Is.True);
        Assert.That(builder.RebuildKey(key, 1, prefix, id) == key, Is.True);
        Assert.That(builder.RebuildKey(key, 2, prefix, id) == key, Is.True);
        Assert.That(builder.RebuildKey(key, 3, prefix, id) == key, Is.True);
    ...
    public void Key8Test()
    {
        ...
        var key = builder.RebuildKey(null, 0, 1, 2, 3, 4, 5, 6, 7, 8);

        Assert.That(key.SequenceEqual(U8("1:2:3:4:5:6:7:8")), Is.True);

        Assert.That(builder.RebuildKey(key, 255, 8, 7, 6, 5, 4, 3, 2, 1) == key, Is.True);
    ...
    public void KeysTest()
    {
        ...
        Assert.That(builder
            .RebuildKey(null, 255, 1)
            .SequenceEqual(U8("1")), Is.True);

        Assert.That(builder
            .RebuildKey(null, 255, 1, 2)
            .SequenceEqual(U8("1:2")), Is.True);

        Assert.That(builder
            .RebuildKey(null, 255, 1, 2, 3)
            .SequenceEqual(U8("1:2:3")), Is.True);

        Assert.That(builder
            .RebuildKey(null, 255, 1, 2, 3, 4)
            .SequenceEqual(U8("1:2:3:4")), Is.True);

        Assert.That(builder
            .RebuildKey(null, 255, 1, 2, 3, 4, 5)
            .SequenceEqual(U8("1:2:3:4:5")), Is.True);

        Assert.That(builder
            .RebuildKey(null, 255, 1, 2, 3, 4, 5, 6)
            .SequenceEqual(U8("1:2:3:4:5:6")), Is.True);

        Assert.That(builder
            .RebuildKey(null, 255, 1, 2, 3, 4, 5, 6, 7)
            .SequenceEqual(U8("1:2:3:4:5:6:7")), Is.True);

        Assert.That(builder
            .RebuildKey(null, 255, 1, 2, 3, 4, 5, 6, 7, 8)
    ...
    {
        AllocForByteTest(KeyRebuilder.Default, 2);
        AllocForByteTest(KeyRebuilder.Fixed, 0, "d3");
    }
    ...
    public void RedisKeyModifyTest()
    {
        ...
        var key = builder.RebuildKey(null, 0, prefix, (sbyte)0);
        ...
        builder.RebuildKey(key, 2, prefix, (sbyte)1);
    ...
    public void KeyFixed()
    {
        ...
        Assert.That(builder.RebuildKey(null, 1, 0).SequenceEqual(U8("0000000000")), Is.True);
        Assert.That(builder.RebuildKey(null, 1, int.MaxValue).SequenceEqual(U8("2147483647")), Is.True);
        Assert.That(builder.RebuildKey(null, 1, uint.MaxValue).SequenceEqual(U8("4294967295")), Is.True);

        Assert.That(builder.RebuildKey(null, 1, (short)0).SequenceEqual(U8("00000")), Is.True);
        Assert.That(builder.RebuildKey(null, 1, short.MaxValue).SequenceEqual(U8("32767")), Is.True);
        Assert.That(builder.RebuildKey(null, 1, ushort.MaxValue).SequenceEqual(U8("65535")), Is.True);

        Assert.That(builder.RebuildKey(null, 1, (byte)0).SequenceEqual(U8("000")), Is.True);
        Assert.That(builder.RebuildKey(null, 1, byte.MaxValue).SequenceEqual(U8("255")), Is.True);
        Assert.That(builder.RebuildKey(null, 1, sbyte.MaxValue).SequenceEqual(U8("127")), Is.True);
    }
    ...
    private void AllocForByteTest(IKeyRebuilder builder, int allocations, string? numFormat = null)
    {
        var prefix = U8("prefix");
        var key = builder.RebuildKey(null, 0, prefix, (byte)0);
        ...
        {
            var newKey = builder.RebuildKey(key, 2, prefix, i);
